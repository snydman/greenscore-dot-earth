      ...
      <div className="mx-auto flex w-full max-w-4xl flex-col gap-8">
        <header className="flex items-center justify-between text-xs sm:text-sm text-[color:var(--gs-text-muted)]">
          ...
          <span className="text-[0.7rem]">
            Placeholder results based on example inputs.
          </span>
        </header>

        <div className="grid gap-6 md:grid-cols-[minmax(0,1.6fr),minmax(0,1.4fr)] md:items-start">
          <Card className="flex flex-col items-center gap-6 text-center border border-[color:var(--gs-border-subtle)]/80">
            <div className="space-y-2">
              <h1 className="text-xl font-semibold tracking-tight text-[color:var(--gs-text-main)] sm:text-2xl">
                Your GreenScore (example)
              </h1>
              <p className="text-xs text-[color:var(--gs-text-muted)]">
                This is a placeholder score to illustrate the experience —
                it&apos;s not based on live data yet.
              </p>
            </div>
            ...
            <div className="relative flex h-44 w-44 items-center justify-center">
              ...
              <div className="relative flex flex-col items-center justify-center gap-1">
                <span className="text-4xl font-semibold text-[color:var(--gs-text-main)] sm:text-5xl">
                  {MOCK_SCORE}
                </span>
          ...
          <div className="space-y-4">
            <Card className="space-y-3 border border-[color:var(--gs-border-subtle)]/80">
                ...
                {CATEGORIES.map((cat) => (
                  ...
                  >
                    <div className="flex flex-col">
                      <span className="font-medium text-[color:var(--gs-text-main)]">
                        {cat.name}
                      </span>
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-semibold text-[color:var(--gs-text-main)]">
                        {cat.score}
                      </span>
            ...
            <Card className="space-y-3 border border-[color:var(--gs-border-subtle)]/80">
              ...
              <ul className="space-y-3 text-sm text-[color:var(--gs-text-main)]">
                {IMPROVEMENTS.map((item) => (
                  <li key={item.title} className="flex gap-3">
                    ...
                    <div className="space-y-1">
                      <p className="font-medium text-[color:var(--gs-text-main)]">
                        {item.title}
                      </p>
                      <p className="text-xs text-[color:var(--gs-text-muted)]">
                        {item.body}
